...
class Processor(CursorWrapper):
    def __init__(self, cur):
        ...
        self.simple_size_limit = int(get_option("simple_size_limit", "100000"))
...
order by url""")
    ...
    def simplify(self, url, url_id):
        ...
        try:
            ...
            sz = 0
            for s in sentences:
                ln = self.make_line(known_words, s)
                if ln is not None:
                    sz += len(ln)
    ...
    def make_line(self, known_words, sentence):
        lst = []
        for token in sentence.xpath("./token"):
            ...
            if tag and pos_rx.match(tag[0]):
                ...
                if sgm:
                    w = sgm[0]
                    if (len(w) > 1) and (w not in known_words):
                        known_words.add(w)
                        lst.append(w)
